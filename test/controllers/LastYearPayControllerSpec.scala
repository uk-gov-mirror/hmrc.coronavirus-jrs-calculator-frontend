...
class LastYearPayControllerSpec extends SpecBaseControllerSpecs with MockitoSugar {
  ...
  val controller = new LastYearPayController(
    messagesApi,
  ...
  "LastYearPay Controller" must {
    ...
    "redirect to error page for GET when index is not valid" when {
      ...
      "index is negative" in {
        when(mockSessionRepository.get(any())) thenReturn Future.successful(Some(variableMonthlyUserAnswers))

        val result = controller.onPageLoad(-1)(getRequest(-1))
      ...
      "index is 0" in {
        ...
        val result = controller.onPageLoad(0)(getRequest(0))
      ...
      "index is too high" in {
        ...
        val result = controller.onPageLoad(3)(getRequest(3))
