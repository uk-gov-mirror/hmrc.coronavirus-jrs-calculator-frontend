...
trait UserAnswersEntryGenerators extends PageGenerators with ModelGenerators {
  ...
  implicit lazy val arbitraryHasEmployeeBeenOnStatutoryLeaveUserAnswersEntry
    : Arbitrary[(HasEmployeeBeenOnStatutoryLeavePage.type, JsValue)] =
    Arbitrary {
      for {
        page  <- arbitrary[HasEmployeeBeenOnStatutoryLeavePage.type]
        value <- arbitrary[Boolean].map(Json.toJson(_))
      } yield (page, value)
    }

  implicit lazy val arbitraryStatutoryLeavePayUserAnswersEntry: Arbitrary[(StatutoryLeavePayPage.type, JsValue)] =
    Arbitrary {
      for {
  ...
  implicit lazy val arbitraryOnPayrollBefore30thOct2020UserAnswersEntry: Arbitrary[(OnPayrollBefore30thOct2020Page.type, JsValue)] =
    Arbitrary {
      for {
        page  <- arbitrary[OnPayrollBefore30thOct2020Page.type]
        value <- arbitrary[Boolean].map(Json.toJson(_))
      } yield (page, value)
    }

  implicit lazy val arbitraryPreviousFurloughPeriodsUserAnswersEntry: Arbitrary[(PreviousFurloughPeriodsPage.type, JsValue)] =
    Arbitrary {
      for {
        page  <- arbitrary[PreviousFurloughPeriodsPage.type]
        value <- arbitrary[Boolean].map(Json.toJson(_))
      } yield (page, value)
    }

  implicit lazy val arbitraryFirstFurloughDateUserAnswersEntry: Arbitrary[(FirstFurloughDatePage.type, JsValue)] =
    Arbitrary {
      for {
        page  <- arbitrary[FirstFurloughDatePage.type]
        value <- arbitrary[Int].map(Json.toJson(_))
      } yield (page, value)
    }

  implicit lazy val arbitraryFurloughInLastTaxYearUserAnswersEntry: Arbitrary[(FurloughInLastTaxYearPage.type, JsValue)] =
    Arbitrary {
      for {
        page  <- arbitrary[FurloughInLastTaxYearPage.type]
        value <- arbitrary[Boolean].map(Json.toJson(_))
      } yield (page, value)
    }
