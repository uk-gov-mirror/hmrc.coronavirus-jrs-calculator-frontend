...
class FurloughPeriodQuestionSpec extends WordSpec with MustMatchers with ScalaCheckPropertyChecks with OptionValues {
  ...
  "FurloughPeriodQuestion" must {
    ...
    "fail to deserialise invalid values" in {
      ...
      val gen = arbitrary[String] suchThat (!FurloughPeriodQuestion.values.map(_.toString).contains(_))

      forAll(gen) { invalidValue =>
        JsString(invalidValue).validate[FurloughPeriodQuestion] mustEqual JsError("error.invalid")
      }
    }
