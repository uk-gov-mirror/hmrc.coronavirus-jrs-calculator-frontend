...
sealed trait PayPeriodQuestion
...
object PayPeriodQuestion extends Enumerable.Implicits {
  ...
  val values: Seq[PayPeriodQuestion] = Seq(
    UseSamePayPeriod,
    UseDifferentPayPeriod
  )

  def options(form: Form[_])(implicit messages: Messages): Seq[RadioItem] = values.map { value =>
    RadioItem(
      value = Some(value.toString),
      content = Text(messages(s"payPeriodQuestion.${value.toString}")),
      checked = form("value").value.contains(value.toString)
    )
  }

  implicit val enumerable: Enumerable[PayPeriodQuestion] =
