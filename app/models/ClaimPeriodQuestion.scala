...
sealed trait ClaimPeriodQuestion
...
object ClaimPeriodQuestion extends Enumerable.Implicits {
  ...
  val values: Seq[ClaimPeriodQuestion] = Seq(
    ClaimOnSamePeriod,
    ClaimOnDifferentPeriod
  )

  def options(form: Form[_])(implicit messages: Messages): Seq[RadioItem] = values.map { value =>
    RadioItem(
      value = Some(value.toString),
      content = Text(messages(s"claimPeriodQuestion.${value.toString}")),
      checked = form("value").value.contains(value.toString)
    )
  }

  implicit val enumerable: Enumerable[ClaimPeriodQuestion] =
