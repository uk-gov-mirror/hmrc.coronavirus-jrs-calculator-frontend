...
sealed trait EmployeeRTISubmission
...
object EmployeeRTISubmission extends Enumerable.Implicits {
  ...
  def options(form: Form[_])(implicit messages: Messages): Seq[RadioItem] = values.map { value =>
    RadioItem(
      ...
      content = Text(messages(s"employeeRTISubmission.${value.toString}")),
      checked = form("value").value.contains(value.toString)
    )
  }

  implicit val enumerable: Enumerable[EmployeeRTISubmission] =
